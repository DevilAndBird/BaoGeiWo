//
//  GTPlaceThirdOrderViewController.swift
//  BaoGeiWo
//
//  Created by wb on 2018/12/26.
//  Copyright © 2018 qyqs. All rights reserved.
//

import UIKit

class GTPlaceThirdOrderViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, WQPickerViewDelegate, GTPlaceOrderCellDelegate, GTPlaceOrderNotesCellDelegate {
    ...
    func setTotalPrice() {
        self.placeOrderModel.totalPrice = "0"
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case (0, 1):
            ...
            pickerVC.style = .enterprise_counter
        ...
        case (1, 5):
            ...
            if receive_type == .airport {
                OrderRequest_swift.getCounterByProvinceId(placeOrderModel.destProvinceId, cityId: placeOrderModel.destCityId, success: { (counters) in
                    let pickerVC = WQPickerViewController()
    ...
    func selectCounter(counter: BGWAirportCounterModel) {
        ...
        placeOrderModel.destCoordinate = counter.counterCoordinate
//        placeOrderModel.destCoordinate = try! String(data: JSONSerialization.data(withJSONObject: counter.counterCoordinate, options: []), encoding: String.Encoding.utf8)!
    ...
    func selectEnterpriseCounter(counter: BGWAirportCounterModel) {
        placeOrderModel.sendAddress = counter.counterName + "-" + counter.counterRemark
        self.updateItemDetailTexts()
        placeOrderModel.srcAddressId = counter.counterId
        placeOrderModel.srcLandmark = counter.counterName + counter.counterRemark
        placeOrderModel.srcAddress = placeOrderModel.srcLandmark
        placeOrderModel.srcCoordinate = counter.counterCoordinate
//        placeOrderModel.srcCoordinate = try! String(data: JSONSerialization.data(withJSONObject: counter.counterCoordinate, options: []), encoding: String.Encoding.utf8)!
        print(placeOrderModel.srcCoordinate)
    }
    ...
    override func viewDidLoad() {
        ...
        if BGWUser.current()?.userRole.cityId == "330100" { //杭州
            placeOrderModel.safeTag = 1             //报价默认为0
            self.selectReceiveType(type: "住宅");    //默认住宅
        } else {
...
//        self.updateItemDetailTexts()
    ...
    func commitOrder() {
        ...
        if placeOrderModel.judgeBaggages(baggages) {
            let para = self.placeOrderModel.placeModelParameter(baggages, "WAITPAY")
...
extension Date {
    ...
    //    func selectDate(_ date: String, displayDate: String, hour: String, minute: String, style: PickerStyle)
    static func mySendDate() -> [String] {
        return self.myCurrentDate(0)
    }
    static func myReceiveDate() -> [String] {
        return self.myCurrentDate(4)
    }
    
    static func myCurrentDate(_ delayHour: Int) -> [String] {
        ...
        if !hourString.isEmpty {
            currentHour = Int(hourString)!
        }
        
        formatStr = "mm"
        dateFormatter.dateFormat = formatStr
        let minuteString = dateFormatter.string(from: currentDate)
        ...
        currentHour += delayHour
        if currentHour > 23 {
            currentHour -= 24
            // day+1
            var newDateComp = DateComponents()
            newDateComp.day = 1
            currentDate = Calendar.current.date(byAdding: newDateComp, to: currentDate)!
        }
        
        if currentHour == 0 {
            hour = "0"
        } else {
            hour = currentHour.description
        }
        ...
        return [yearString, dateString, hour, minute]
    }
