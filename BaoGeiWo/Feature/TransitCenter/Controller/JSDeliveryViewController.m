...
@interface JSDeliveryViewController ()<UITableViewDataSource, UITableViewDelegate>
...
@implementation JSDeliveryViewController
...
- (void)viewDidLoad {
    ...
    self.navigationBar.titleLabel.text = @"集散中心交接";
...
- (void)selectActionType:(UISegmentedControl *)sender {
    ...
    if (sender.selectedSegmentIndex == 0) {
        ...
        [self.deliveButton setTitle:@"申请装车交接" forState:UIControlStateNormal];
    }
    [self getOrderListData];
}
...
- (void)deliverClick {
    ...
    if (self.roleActionType == BGWRoleActionTypeTransitSend || self.roleActionType == BGWRoleActionTypeAirportSend) {
        ...
        [OrderRequest transitCenterUnloadScanWithDeliveryId:self.deliveryMan.dmId destId:[BGWUser currentUser].userRole.userRoleId success:^(id responseObject) {
